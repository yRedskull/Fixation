...
def select_dir(self, bag, fog, fts, bg_app):
        ...
        self.btn_1.pack(ipady=3, ipadx=3, pady=1, padx=2, side="left")
        ...
        self.direc = Button(self.frame_2, image=self.img_explorer, command=lambda: direct(self), highlightthickness=0, bd=0, bg=bag)
        self.direc.grid(column=6, row=2, ipadx=3, ipady=3)

        def configbgcolor(event):
            self.direc.config(bg=bg_app)
            return event
        ...
        self.run = Button(self.frame_2, image=self.img_next, command=lambda: add_folders(self, bag, fog, fts, bg_app), highlightthickness=0, bd=0,
                          bg=bag)
        self.run.grid(column=5, row=3, ipadx=3, ipady=3)
        ...
        self.btn_github = Button(self.app, image=self.img_github, command=lambda: browser.open('https://github.com/yRedskull/Fixation'), font=fts, bg=bg_app, fg=fog, bd=0, highlightthickness=0)
